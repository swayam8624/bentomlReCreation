...
def plot_gpu_telemetry_timeline(gpu_history: List[GPUState], 
    ...
    memory_used = [s.memory_used_mb / 1024 for s in gpu_history]  # Convert to GB
    ...
    # PANEL 2: Power Draw
    # ═══════════════════════════════════════════════════════════════════════
    axes[1].plot(timestamps, power_draw, linewidth=2, color='#FF6B35', label='Power Draw')
    axes[1].fill_between(timestamps, power_draw, alpha=0.3, color='#FF6B35')
    
    for start, end, level in load_transitions:
        axes[1].axvspan(start, end, alpha=0.15, color='#FFA500')
    
    avg_power = statistics.mean(power_draw)
    axes[1].axhline(y=avg_power, color='red', linestyle='--', linewidth=1.5, 
                   label=f'Avg: {avg_power:.1f}W')
    
    axes[1].set_ylabel('Power Draw (W)', fontweight='bold', fontsize=11)
    axes[1].set_title('GPU Power Consumption', fontweight='bold', fontsize=12)
    ...
    # ═══════════════════════════════════════════════════════════════════════
    axes[2].plot(timestamps, memory_used, linewidth=2, color='#4A90E2', label='VRAM Used')
    axes[2].fill_between(timestamps, memory_used, alpha=0.3, color='#4A90E2')
    
    for start, end, level in load_transitions:
        axes[2].axvspan(start, end, alpha=0.15, color='#FFA500')
    
    peak_memory = max(memory_used)
    axes[2].axhline(y=peak_memory, color='red', linestyle='--', linewidth=1.5,
                   label=f'Peak: {peak_memory:.1f} GB')
    
    axes[2].set_ylabel('VRAM Used (GB)', fontweight='bold', fontsize=11)
    axes[2].set_title('GPU Memory Utilization', fontweight='bold', fontsize=12)
    ...
    # PANEL 4: Temperature
    # ═══════════════════════════════════════════════════════════════════════
    axes[3].plot(timestamps, temperature, linewidth=2, color='#FF1493', label='GPU Temp')
    axes[3].fill_between(timestamps, temperature, alpha=0.3, color='#FF1493')
    
    for start, end, level in load_transitions:
        axes[3].axvspan(start, end, alpha=0.15, color='#FFA500')
    
    max_temp = max(temperature)
    axes[3].axhline(y=max_temp, color='red', linestyle='--', linewidth=1.5,
                   label=f'Peak: {max_temp:.1f}°C')
    
    # Throttling warning line (typically 83°C for A40)
    axes[3].axhline(y=83, color='orange', linestyle=':', linewidth=2, 
                   label='Throttle Threshold: 83°C', alpha=0.7)
    
    axes[3].set_xlabel('Time (seconds)', fontweight='bold', fontsize=12)
    axes[3].set_ylabel('Temperature (°C)', fontweight='bold', fontsize=11)
    axes[3].set_title('GPU Temperature', fontweight='bold', fontsize=12)
    axes[3].grid(True, alpha=0.3)
    axes[3].legend(loc='upper left')
    
    # Overall title
    fig.suptitle(f'GPU Telemetry Timeline: {gpu_name}\n'
                f'Kernel Occupancy vs. Power vs. Memory vs. Temperature',
